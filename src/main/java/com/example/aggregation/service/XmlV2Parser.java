...
/**
 * Implementation of {@Parser} that uses data loaded from XML ver.2 source.
 */
@Service
@RequiredArgsConstructor
public class XmlV2Parser implements Parser {
    private final static Logger logger = LoggerFactory.getLogger(XmlV2Parser.class);
    private final SourceProperties properties;

    /**
     * Parse xml ver.2 document
        ...
        try {
            ...
            for (int temp = 0; temp < list.getLength(); temp++) {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    currency = element.getElementsByTagName("Price").item(0).getAttributes().item(0).getTextContent();
                    bestBefore = element.getElementsByTagName("BestBefore").item(0).getAttributes().item(0).getTextContent();
    ...
    public String getSource() {
        return properties.getSourceConfigs().get(properties.getSource2()).getUrl();
    }
}
